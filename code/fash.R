...
EMest_logF = function(logfhat,df1,df2,g,prior,null.comp=1,nullcheck=TRUE,control=list()){ 
  ...
  if(nullcheck==TRUE){ #null check doesn't work with VB yet
    pinull = rep(0,k)
    pinull[null.comp]=1
...
my_etruncf_vec = function(alpha_beta,v1,v2,log=FALSE){
  tmp = apply(alpha_beta,1,my_etruncf,v1=v1,v2=v2,
...
my_etruncf= function(a_b,v1,v2,lowthres,highthres){
  ...
  if (a==b){
    ...
    if (b<=highthres & a>=lowthres){
      tmp = extrunc(spec="f", df1=v1, df2=v2, a=a, b=b)
    }else{
...
etruncf_num = function(x,df1,df2,a,b){
  #c = 10^(-round(min(log10(df(a,df1,df2)),log10(df(b,df1,df2)))))
...
thresq = function(thres,df1,df2){
  highthresq = (thres*beta(df1/2,df2/2)*(df1/df2)^(df2/2))^(-1/(df2/2+1))
  lowthresq = (thres*beta(df1/2,df2/2)*(df2/df1)^(df1/2))^(1/(df1/2-1))
  return(list(high=highthresq, low=lowthresq))
}
...
my_etrunclogf_vec = function(alpha_beta,v1,v2,log=FALSE){
  tmp = apply(alpha_beta,1,my_etrunclogf,v1=v1,v2=v2,
